...
@Component({
  selector: 'app-parrucchiere',
  templateUrl: './parrucchiere.page.html',
  styleUrls: ['./parrucchiere.page.scss'],
})
export class ParrucchierePage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.shopList = this.shopList.filter(currentFood => {
      if (currentFood.title && "Parrucchiere") {
        return (currentFood.title.toLowerCase().indexOf("Parrucchiere".toLowerCase()) > -1 || currentFood.description.toLowerCase().indexOf("Parrucchiere".toLowerCase()) > -1
        || currentFood.description.toLowerCase().indexOf("Barbiere".toLowerCase()) > -1);
      }
    });
    this.distance();
  }
